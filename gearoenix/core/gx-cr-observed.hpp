...
public:
    ...
    explicit Observed(Args&&... args) noexcept
    ...
    [[nodiscard]] std::size_t add_observer(std::function<bool(const T&)>&& f) noexcept
    {
    ...
    void remove_listener(const std::size_t id) noexcept
    {
        if (id == 0)
            return;
        listeners.erase(id);
