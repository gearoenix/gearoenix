...
#define GX_GETSET_UPTR(v, t, x) GX_GET_UPTR(v,, t, x) void set_##x(std::unique_ptr<t> _##x) noexcept { x = std::move(_##x); }
...
#define GX_GETSET_PTR(v, t, x) GX_GET_PTR(v, t, x) void set_##x(t * const _##x) noexcept { x = _##x; }
...
#define GX_CHECK_NOT_EQAUL(x, y) { if ((x) == (y)) { GXLOGF("Unequality check failed.") } }
