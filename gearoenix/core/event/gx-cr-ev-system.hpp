...
struct WindowSizeChangeData final {
    ...
    const core::fp_t previous_reversed_half_width;
    const core::fp_t previous_reversed_half_height;
    const core::fp_t previous_ratio;
    const int current_width;
    const int current_height;
    const core::fp_t current_reversed_half_width;
    const core::fp_t current_reversed_half_height;
    const core::fp_t current_ratio;
    const int delta_width;
    const int delta_height;
    const core::fp_t delta_reversed_half_width;
    const core::fp_t delta_reversed_half_height;
    const core::fp_t delta_ratio;
    ...
    WindowSizeChangeData(const int previous_width, const int previous_height, const int current_width, const int current_height)
        ...
        , previous_reversed_half_width(2.0 / static_cast<core::fp_t>(previous_width))
        , previous_reversed_half_height(2.0 / static_cast<core::fp_t>(previous_height))
        , previous_ratio(previous_reversed_half_height / previous_reversed_half_width)
        , current_width(current_width)
        , current_height(current_height)
        , current_reversed_half_width(2.0 / static_cast<core::fp_t>(current_width))
        , current_reversed_half_height(2.0 / static_cast<core::fp_t>(current_height))
