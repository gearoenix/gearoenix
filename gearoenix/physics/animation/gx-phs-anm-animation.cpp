...
void gearoenix::physics::animation::ArmatureAnimation::animate(const core::fp_t time) { animate(*root_bone, time); }
...
gearoenix::physics::animation::SpriteAnimation::SpriteAnimation(std::string&& name, std::shared_ptr<render::material::Sprite>&& sprite, const std::uint32_t width, const std::uint32_t height)
    ...
    , aspect(width, height)
    , uv_scale(1.0 / static_cast<core::fp_t>(width), 1.0f / static_cast<core::fp_t>(height))
{
}
