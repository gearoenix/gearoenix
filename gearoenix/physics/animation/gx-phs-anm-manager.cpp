...
gearoenix::core::ecs::EntityPtr gearoenix::physics::animation::Manager::create_armature(std::string&& name, core::ecs::Entity* const parent, std::shared_ptr<Bone>&& root_bone)
{
    ...
    auto transform = core::Object::construct<Transformation>(entity.get(), entity->get_object_name() + "-transformation");

    auto anim = core::Object::construct<ArmatureAnimation>(entity->get_object_name() + "-animation", std::shared_ptr(root_bone));

    auto armature = core::Object::construct<Armature>(entity.get(), entity->get_object_name() + "-armature", transform.get());
...
{
    core::ecs::World::get().parallel_system<AnimationPlayer>([](auto, auto* const player, auto) {
