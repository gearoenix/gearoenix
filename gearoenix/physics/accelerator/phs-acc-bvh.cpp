    ...
    for(const auto di: dimi) {
        ...
        for (const collider::Collider* c : colliders) {
            ...
            if(not_found) {
                ...
                bins[WALLS_COUNT].b.put(c->get_box());
            }
        }
        ...
        ++best_wall_index;
        std::vector<const collider::Collider*> left_colliders;
        left_colliders.reserve(best_wall_left_count);
        for (int i = 0; i < best_wall_index; ++i)
            for (auto c : bins[i].c)
                left_colliders.push_back(c);
        std::vector<const collider::Collider*> right_colliders;
        right_colliders.reserve(best_wall_right_count);
        for (int i = best_wall_index; i < BINS_COUNT; ++i)
            for (auto c : bins[i].c)
