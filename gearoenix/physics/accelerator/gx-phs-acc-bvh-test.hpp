...
BOOST_AUTO_TEST_CASE(gearoenix_accelerator_bvh_ray_hit)
{
    ...
    using Box = gearoenix::math::Aabb3<double>;
    using V3 = gearoenix::math::Vec3<double>;
    using Ray = gearoenix::math::Ray3<double>;

    gearoenix::physics::accelerator::Bvh<Data> bvh;

    /// Place 5 boxes along the +Z axis at z = 2, 4, 6, 8, 10
    for (int i = 0; i < 5; ++i) {
        const double z = 2.0 + 2.0 * static_cast<double>(i);
    ...
    {
        gearoenix::physics::accelerator::Bvh<Data> empty_bvh;
        const Ray ray(V3(0.0), V3(0.0, 0.0, 1.0));
...
BOOST_AUTO_TEST_CASE(gearoenix_accelerator_bvh_sah_quality)
{
    ...
    for (int i = 0; i < grid - 1; ++i) {
        for (int j = 0; j < grid - 1; ++j) {
            const double x = static_cast<double>(i) * 3.0 + 1.5;
            const double y = static_cast<double>(j) * 3.0 + 1.5;
            const Ray ray(V3(x, y, 100.0), V3(0.0, 0.0, -1.0));
            const auto result = bvh.hit(ray, 1000.0);
            BOOST_TEST(!result.has_value());
        }
...
BOOST_AUTO_TEST_CASE(gearoenix_accelerator_bvh_sah_depth_layers)
{
    ...
    for (int layer = 0; layer < 4; ++layer) {
        for (int i = 0; i < 4; ++i) {
            const double x = static_cast<double>(i) * 3.0;
            const double z = static_cast<double>(layer) * 5.0;
...
BOOST_AUTO_TEST_CASE(gearoenix_accelerator_bvh_coincident_centers)
{
    ...
    gearoenix::physics::accelerator::Bvh<Data> bvh;
    for (int i = 0; i < 20; ++i) {
        const double half = 1.0 + static_cast<double>(i) * 0.1;
...
BOOST_AUTO_TEST_CASE(gearoenix_accelerator_bvh_diagonal_ray)
{
    ...
    const V3 grid_center(
        static_cast<double>(grid - 1) * spacing * 0.5,
        static_cast<double>(grid - 1) * spacing * 0.5,
        static_cast<double>(grid - 1) * spacing * 0.5);
