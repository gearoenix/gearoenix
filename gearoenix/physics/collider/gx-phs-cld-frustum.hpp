...
struct Frustum final : Collider {
    ...
    constexpr static auto default_points = std::array {
        math::Vec3<core::fp_t>(-1.0, 1.0, 1.0),
        math::Vec3<core::fp_t>(1.0, 1.0, 1.0),
        math::Vec3<core::fp_t>(-1.0, 1.0, -1.0),
        math::Vec3<core::fp_t>(1.0, 1.0, -1.0),
        math::Vec3<core::fp_t>(-1.0, -1.0, 1.0),
    ...
    GX_GET_CREF_PRV(math::Frustum<core::fp_t>, frustum);

    void update_surrounding_box() override;

    Frustum(core::ecs::Entity* entity, std::shared_ptr<Transformation>&& transform, std::string&& name, const std::array<math::Vec3<core::fp_t>, 8>& points);

public:
    /// For the arrangement of point go to the math::Frustum constructor
    void update(const std::array<math::Vec3<core::fp_t>, 8>& points);
    [[nodiscard]] bool check_intersection(const math::Aabb3<core::fp_t>& box) const override;
    [[nodiscard]] math::IntersectionStatus check_intersection_status(const math::Aabb3<core::fp_t>& box) const override;
};
}