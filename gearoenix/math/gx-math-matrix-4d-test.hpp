...
BOOST_AUTO_TEST_CASE(gearoenix_math_matrix_4d)
{
    ...
    auto compare = [&](const std::string& operation, const std::source_location location = std::source_location::current()) noexcept {
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j) {
                ...
                if (d / (a + b) < d)
                    continue;
                BOOST_TEST_FAIL("Mat4x4 compare failed on [" << i << "][" << j << "] in line: " << location.line() << " in operation: " << operation);
            }
    };
    ...
    compare("hard-coded-rotation-translation-1");
