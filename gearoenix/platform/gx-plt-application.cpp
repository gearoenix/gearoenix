...
{
    return (x - static_cast<core::fp_t>(window_size.x) * static_cast<core::fp_t>(0.5)) / static_cast<core::fp_t>(window_size.y);
}

gearoenix::core::fp_t gearoenix::platform::BaseApplication::normalise_y(const core::fp_t y) const
{
    return static_cast<core::fp_t>(0.5) - (y / static_cast<core::fp_t>(window_size.y));
}
