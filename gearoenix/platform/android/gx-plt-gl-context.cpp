...
bool gearoenix::platform::GlContext::check_surface(const EGLint opengl_version, const EGLint depth_size, const EGLint samples_size) noexcept
{
...
void gearoenix::platform::GlContext::init(ANativeWindow* const window) noexcept
{
...
void gearoenix::platform::GlContext::resume(ANativeWindow* const window) noexcept
{
    if (!egl_context_initialized) {
        init(window);
        return;
