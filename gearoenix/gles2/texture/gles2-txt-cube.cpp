...
gearoenix::gles2::texture::Cube::Cube(system::File* file, Engine* eng, std::shared_ptr<core::EndCaller> end)
{
    ...
    std::function<void()> loadf = [this, imgw, imgh, imgc, img_data, end] {
        ...
        for (int i = 0; i < FACES_COUNT; ++i) {
            switch (imgc[i]) {
            case 4:
                glTexImage2D(GL_TEXTURE_CUBE_MAP, 0, GL_RGBA, imgw[i], imgh[i], 0, GL_RGBA, GL_UNSIGNED_BYTE, (void*)img_data[i].data());
                break;
            case 3:
                glTexImage2D(GL_TEXTURE_CUBE_MAP, 0, GL_RGB, imgw[i], imgh[i], 0, GL_RGB, GL_UNSIGNED_BYTE, (void*)img_data[i].data());
                break;
            default:
                UNEXPECTED;
            }
