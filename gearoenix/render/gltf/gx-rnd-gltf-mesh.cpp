...
void load_mesh(const Context& context, const int mesh_index, const int primitive_index, const core::job::EndCaller<>& end_callback)
{
    ...
    const math::Aabb3 box(
        math::Vec3(
            static_cast<core::fp_t>(pos_max[0]),
            static_cast<core::fp_t>(pos_max[1]),
            static_cast<core::fp_t>(pos_max[2])),
        math::Vec3(
            static_cast<core::fp_t>(pos_min[0]),
            static_cast<core::fp_t>(pos_min[1]),
            static_cast<core::fp_t>(pos_min[2])));
...
bool gearoenix::render::gltf::Meshes::process(const int node_index, core::ecs::Entity* const parent) const
{
    ...
    auto arm = parent != nullptr ? parent->get_component_shared_ptr<physics::animation::Armature>() : nullptr;
    GX_ASSERT_D((node.skin != -1) == (arm != nullptr));
    auto entity = model::Manager::get().build(std::string(node.name), parent, std::move(meshes_set), true, std::move(arm));
