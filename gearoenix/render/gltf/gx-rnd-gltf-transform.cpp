...
void gearoenix::render::gltf::apply_transform(const int node_index, const Context& context, physics::Transformation& transform)
{
    ...
    if (scale.size() == 3) {
        transform.local_inner_scale(math::Vec3<core::fp_t>(scale[0], scale[1], scale[2]));
    }
    if (rotation.size() == 4) {
        transform.local_inner_rotate(math::Quat<core::fp_t>(rotation[0], rotation[1], rotation[2], rotation[3]));
    }
    if (translation.size() == 3) {
        transform.set_local_position(math::Vec3<core::fp_t>(translation[0], translation[1], translation[2]));
    }

    if (!node.matrix.empty()) {
        const auto local_matrix = math::Mat4x4<core::fp_t>(
