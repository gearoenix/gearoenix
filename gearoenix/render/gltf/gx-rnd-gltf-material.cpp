...
void gearoenix::render::gltf::Materials::initialise(const int index) const
{
    ...
    gx_mat.set_emission_roughness_factor({ static_cast<float>(mat.emissiveFactor[0]),
    ...
    gx_mat.set_normal_metallic_factor({ static_cast<float>(mat.normalTexture.scale),
    ...
    gx_mat.set_albedo_factor({ static_cast<float>(albedo_factor[0]),
