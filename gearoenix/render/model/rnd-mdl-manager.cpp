...
std::shared_ptr<gearoenix::render::model::Model> gearoenix::render::model::Manager::get_gx3d(const core::Id id, core::sync::EndCaller<Model>& c) noexcept
{
    std::shared_ptr<Model> m = cache.get<Model>(
        id, [id, c, this](std::string name) noexcept {
            ...
            switch (t) {
            case Type::Dynamic:
                return std::shared_ptr<Model>(new Dynamic(id, std::move(name), f, e, call));
            case Type::Static:
                return std::shared_ptr<Model>(new Static(id, std::move(name), f, e, call));
            default:
                return std::dynamic_pointer_cast<Model>(widget::Widget::read_gx3d(id, std::move(name), f, e, call));
            }
        });
    c.set_data(m);
    return m;
