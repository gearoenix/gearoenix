...
void gearoenix::render::camera::Manager::build_impl(std::string&& name, core::ecs::Entity* const parent, const core::job::EndCaller<core::ecs::EntityPtr>& entity_callback)
{
    ...
    auto frustum = core::Object::construct<physics::collider::Frustum>(
        entity.get(), std::shared_ptr(transform), entity->get_object_name() + "-collider", physics::collider::Frustum::default_points);
...
void gearoenix::render::camera::Manager::update()
{
    core::ecs::World::get().parallel_system<core::ecs::All<Camera, physics::Transformation, physics::collider::Frustum>>(
        [](const auto /*entity-id*/, auto* const cam, const auto* const transform, auto* const collider, const auto /*kernel_index*/) {
            cam->set_view(transform->get_transposed_inverted_global_matrix().transposed().template to<float>());
...
{
    core::ecs::World::get().parallel_system<core::ecs::All<Camera>>(
        [](const auto, auto* const c, const auto) -> void {
