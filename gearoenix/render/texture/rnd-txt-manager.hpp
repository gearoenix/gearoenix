...
protected:
    ...
    [[nodiscard]] std::shared_ptr<Texture> read_gx3d(core::Id id, system::stream::Stream* s, core::sync::EndCaller<Texture>& c) noexcept;
...
public:
    ...
    [[nodiscard]] std::shared_ptr<Texture2D> create_2d(std::vector<std::vector<std::uint8_t>> data, const TextureInfo& info, std::size_t img_width, std::size_t img_height, core::sync::EndCaller<Texture2D>& c) noexcept;
    /// It creates 2d floating texture from formatted data
    [[nodiscard]] std::shared_ptr<Texture2D> create_2d(const unsigned char* data, std::size_t size, core::sync::EndCaller<Texture2D>& c, const SampleInfo& sample_info = SampleInfo()) noexcept;
    /// It creates 2d floating texture from formatted data for final float images
    [[nodiscard]] std::shared_ptr<Texture2D> create_2d_f(const unsigned char* data, std::size_t size, core::sync::EndCaller<Texture2D>& c, const SampleInfo& sample_info = SampleInfo()) noexcept;
    /// It creates 2d floating texture from file
    [[nodiscard]] std::shared_ptr<Texture2D> create_2d_f(const std::wstring& file_address, core::sync::EndCaller<Texture2D>& c, const SampleInfo& sample_info = SampleInfo()) noexcept;
    [[nodiscard]] std::shared_ptr<Texture2D> create_2d_f(const std::string& file_address, core::sync::EndCaller<Texture2D>& c, const SampleInfo& sample_info = SampleInfo(), bool relative_path = true) noexcept;
    ...
    [[nodiscard]] std::shared_ptr<TextureCube> create_cube(const TextureInfo& info, int img_aspect, core::sync::EndCaller<TextureCube>& c) noexcept;
    ...
    [[nodiscard]] std::shared_ptr<Texture> read_gx3d(system::stream::Stream* s, core::sync::EndCaller<Texture>& c) noexcept;
