...
gearoenix::render::widget::Modal::Modal(std::string&& name, std::shared_ptr<Button>&& close, std::shared_ptr<Button>&& cancel, std::shared_ptr<Button>&& ok)
...
void gearoenix::render::widget::Modal::construct(std::string&& name, std::string&& background_texture_asset, const std::optional<std::pair<std::string, std::string>>& close_button_texture_asset,
    const std::optional<std::pair<std::string, std::string>>& cancel_button_texture_asset, const std::optional<std::pair<std::string, std::string>>& ok_button_texture_asset, core::ecs::Entity* const camera_entity, std::shared_ptr<Widget>&& parent,
    core::job::EndCaller<ConstructorReturn>&& end_callback)
{
    ...
    material::Manager::get().get_unlit(values->return_value.modal->name + "-material", core::job::EndCallerShared<material::Unlit>([values, values_ready](std::shared_ptr<material::Unlit>&& mat) {
        values->mat = std::move(mat);
        mesh::Manager::get().build_plate(std::shared_ptr<material::Material>(values->mat), core::job::EndCallerShared<mesh::Mesh>([values, values_ready](std::shared_ptr<mesh::Mesh>&& msh) {
    ...
    texture::Manager::get().create_2d_from_file(path, texture::TextureInfo(), core::job::EndCallerShared<texture::Texture2D>([values, values_ready](std::shared_ptr<texture::Texture2D>&& tex) {
    ...
    if (cancel_button_texture_asset.has_value()) {
        Button::construct(result->name + "-cancel-button", std::string(cancel_button_texture_asset->first), std::string(cancel_button_texture_asset->second), camera_entity, std::shared_ptr(result),
