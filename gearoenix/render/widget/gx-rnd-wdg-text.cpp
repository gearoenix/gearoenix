    ...
    core::job::EndCallerShared<Text>&& end_callback)
{
    mesh::Manager::get().build_plate(
        std::move(mat),
        core::job::EndCallerShared<mesh::Mesh>([n = std::move(name), c = camera_entity, p = std::move(parent), e = std::move(end_callback)](std::shared_ptr<mesh::Mesh>&& msh) mutable {
...
void gearoenix::render::widget::Text::construct(std::string&& name, core::ecs::Entity* const camera_entity, std::shared_ptr<Widget>&& parent, core::job::EndCallerShared<Text>&& end_callback)
{
    material::Manager::get().get_unlit(
        name + "-material",
        core::job::EndCallerShared<material::Unlit>([n = std::move(name), c = camera_entity, p = std::move(parent), e = std::move(end_callback)](std::shared_ptr<material::Unlit>&& mat) mutable {
    ...
    core::job::EndCallerShared<Text>&& end_callback)
{
    ...
    auto model_entity = model::Manager::get().build(
        r->name + "-model-text", parent ? parent->get_model_entity().get() : nullptr, { std::move(msh) }, true, nullptr);
...
gearoenix::render::widget::Text::~Text() = default;
