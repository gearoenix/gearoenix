	...
	, shadow_mapper_secondary_commands(static_cast<std::size_t>(e->get_system_application()->get_configuration().render_config.shadow_cascades_count))
{
	...
	for (command::Buffer*& b : shadow_mapper_secondary_commands) b = cmdmgr->create_secondary_command_buffer(kernel_index);
}

gearoenix::render::light::KernelCascadeInfo::~KernelCascadeInfo() noexcept
{
	for (command::Buffer* b : shadow_mapper_secondary_commands) delete b;
...
{
	for (std::size_t i = 0; i < kernels.size(); ++i) kernels[i] = new KernelCascadeInfo(e, i);
	command::Manager* cmdmgr = e->get_command_manager();
	for (command::Buffer*& b : shadow_mapper_primary_commands) b = cmdmgr->create_primary_command_buffer();
}

gearoenix::render::light::FrameCascadeInfo::~FrameCascadeInfo() noexcept
{
	for (KernelCascadeInfo* k : kernels) delete k;
	kernels.clear();
	for (command::Buffer* b : shadow_mapper_primary_commands) delete b;
	...
	: e(e)
{
	...
	for (FrameCascadeInfo*& frame : frames) frame = new FrameCascadeInfo(e);
}

gearoenix::render::light::CascadeInfo::~CascadeInfo() noexcept
{
	for (FrameCascadeInfo* frame : frames) delete frame;
	frames.clear();
}
