    ...
    , e(e)
{
	...
	pre_sems.resize(input_links.size());
...
void gearoenix::render::graph::node::Node::remove_cunsomer(const unsigned int output_link_index, const core::Id node_id) noexcept
{
	links_consumers_frames_semaphores[output_link_index][node_id].clear();
	core::graph::Node::remove_cunsomer(output_link_index, node_id);
