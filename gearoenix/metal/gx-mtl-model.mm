...
gearoenix::metal::Model::Model(Engine& e, std::shared_ptr<Mesh>&& bound_mesh, const NSUInteger buffer_size, const std::string& name)
...
gearoenix::metal::ModelBuilder::ModelBuilder(Engine& e, const std::string& name, std::shared_ptr<render::mesh::Mesh>&& bound_mesh, bool is_transformable)
...
void gearoenix::metal::ModelBuilder::set_material(const render::material::Pbr& mat)
{
    ...
    [m.gbuffers_filler_args.encoder setBuffer:e.get_buffer_manager()->uniforms_gpu offset:m.uniform.gpu_offset atIndex:GEAROENIX_METAL_GBUFFERS_FILLER_MODEL_UNIFORM_BIND_ID];
    const Texture2D& at = *dynamic_cast<const Texture2D*>(mat.get_albedo().get());
    [m.gbuffers_filler_args.encoder setSamplerState:at.sampler atIndex:GEAROENIX_METAL_GBUFFERS_FILLER_SAMPLER_BIND_ID];
    [m.gbuffers_filler_args.encoder setTexture:at.resource atIndex:GEAROENIX_METAL_GBUFFERS_FILLER_ALBEDO_TEXTURE_BIND_ID];
    [m.gbuffers_filler_args.encoder setTexture:dynamic_cast<const Texture2D*>(mat.get_normal__metallic().get())->resource atIndex:GEAROENIX_METAL_GBUFFERS_FILLER_NORMAL_METALLIC_TEXTURE_BIND_ID];
    [m.gbuffers_filler_args.encoder setTexture:dynamic_cast<const Texture2D*>(mat.get_emission__roughness().get())->resource atIndex:GEAROENIX_METAL_GBUFFERS_FILLER_EMISSION_ROUGHNESS_TEXTURE_BIND_ID];
    b.add_component(std::move(m));
}

gearoenix::metal::ModelBuilder::~ModelBuilder() = default;

std::shared_ptr<gearoenix::render::model::Builder> gearoenix::metal::ModelManager::build(std::string&& name, std::shared_ptr<render::mesh::Mesh>&& mesh, core::job::EndCaller&&, const bool is_transformable)
{
