...
struct Mesh final : public render::mesh::Mesh {
    ...
    Mesh(id<MTLBuffer> vb, id<MTLBuffer> ib, math::Aabb3<core::fp_t>&& box, NSUInteger vertex_size, NSUInteger vertices_size, NSUInteger indices_count);
    ~Mesh() override;
...
struct MeshManager final : render::mesh::Manager {
    ...
    [[nodiscard]] std::shared_ptr<render::mesh::Mesh> build(
        std::string&& name, std::vector<render::PbrVertex>&& vertices, std::vector<std::uint32_t>&& indices, math::Aabb3<core::fp_t>&& occlusion_box, core::job::EndCaller&& end_callback) override;
};
}
