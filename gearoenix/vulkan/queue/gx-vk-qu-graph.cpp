...
void gearoenix::vulkan::queue::Graph::SubmitData::FrameData::clear()
{
...
void gearoenix::vulkan::queue::Graph::SubmitData::clear()
{
...
void gearoenix::vulkan::queue::Graph::SubmitData::add_provider(const std::vector<std::shared_ptr<sync::Semaphore>>& semaphores)
{
    if (frame_data.empty()) {
    ...
    for (std::uint64_t i = 0; i < frame_data.size(); ++i) {
        ...
        GX_ASSERT_D(std::ranges::find(pw, s) == pw.end());
        pw.push_back(s);
...
void gearoenix::vulkan::queue::Graph::SubmitData::add_consumer(
    const std::vector<std::shared_ptr<sync::Semaphore>>& semaphores)
{
    if (frame_data.empty())
        frame_data.resize(semaphores.size());
    else
    ...
    for (std::uint64_t i = 0; i < frame_data.size(); ++i) {
        ...
        GX_ASSERT_D(std::ranges::find(cs, s) == cs.end());
        cs.push_back(s);
    }
}

void gearoenix::vulkan::queue::Graph::update_traversing_level()
{
...
void gearoenix::vulkan::queue::Graph::update_submit_data(Node& node)
{
...
void gearoenix::vulkan::queue::Graph::update()
{
...
void gearoenix::vulkan::queue::Graph::submit()
{
    const auto frame_index = core::Singleton<engine::Engine>::get().get_frame_number();
    ...
    for (std::uint64_t submit_data_index = 0; submit_data_index < submit_data_end; ++submit_data_index) {
        const auto& [frame_data] = submit_data[submit_data_index];
        const auto& [providers_waits, pipeline_stages, cmd, consumers_signals] = frame_data[frame_index];
        q.submit(
            providers_waits.size(), providers_waits.data(),
            pipeline_stages.data(),
            cmd->get_has_record() ? 1 : 0, cmd->get_vulkan_data_ptr(),
            consumers_signals.size(), consumers_signals.data());
    }
    {
        const auto& [frame_data] = submit_data[submit_data_end];
        const auto& [providers_waits, pipeline_stages, cmd, consumers_signals] = frame_data[frame_index];
        q.submit(
            providers_waits.size(), providers_waits.data(),
            pipeline_stages.data(),
...
gearoenix::vulkan::queue::Graph::Graph()
    : start_semaphore(sync::Semaphore::create_frame_based())
    , end_semaphore(sync::Semaphore::create_frame_based())
    , fence(sync::Fence::create_frame_based(true))
{
    nodes.emplace(NodeLabel::Start, Node(NodeLabel::Start, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT));
    nodes.emplace(NodeLabel::End, Node(NodeLabel::End, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT));
    Node::connect(nodes.find(NodeLabel::Start)->second, nodes.find(NodeLabel::End)->second);
}

gearoenix::vulkan::queue::Graph::~Graph() = default;
    ...
    const VkPipelineStageFlags wait_stage)
{
    ...
    auto& new_node = nodes.emplace(new_node_label, Node(new_node_label, wait_stage)).first->second;
    ...
    Node::connect(provider_node, new_node);
    Node::connect(new_node, consumer_node);
...
void gearoenix::vulkan::queue::Graph::start_frame()
{
    auto& f = *fence[core::Singleton<engine::Engine>::get().get_frame_number()];
    f.wait();
    f.reset();
}

bool gearoenix::vulkan::queue::Graph::present(const std::uint32_t image_index)
{
    submit();
    ...
    info.pWaitSemaphores = end_semaphore[core::Singleton<engine::Engine>::get().get_frame_number()]->get_vulkan_data_ptr();
...
const gearoenix::vulkan::sync::Semaphore& gearoenix::vulkan::queue::Graph::get_present_semaphore()
{
    return *start_semaphore[core::Singleton<engine::Engine>::get().get_frame_number()];
}
