...
//		LOGE(std::string("queue node index added is ") + std::to_string(int(q)));
	}
	...
	VKC(l->vkCreateDevice(
            physical_device->get_vulkan_data(), &device_create_info, 0, &vulkan_data));
	l->vkGetDeviceQueue(
			vulkan_data, physical_device->get_graphics_queue_node_index(), 0, &vk_graphic_queue);
