...
int gearoenix::nufrag::render::device::Physical::is_good(const VkPhysicalDevice &gpu) {
	...
	l->vkGetPhysicalDeviceQueueFamilyProperties(gpu, &queue_count, queue_props.data());
	std::vector<VkBool32> supports_present(queue_count);
	for (uint32_t i = 0; i < queue_count; ++i) {
		l->vkGetPhysicalDeviceSurfaceSupportKHR(gpu, i, surface->get_vulkan_data(), &(supports_present[i]));
	}
	for (uint32_t i = 0; i < queue_count; i++) {
		if (((queue_props[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0) &&
			((queue_props[i].queueFlags & VK_QUEUE_TRANSFER_BIT) != 0) &&
	...
	LOGE(std::string("Separate graphics and presenting queues are not supported yet!"));
	return -1;
}
...
gearoenix::nufrag::render::device::Physical::Physical(const std::shared_ptr<Surface> &surface): surface(surface) {
	...
	VKC(l->vkEnumeratePhysicalDevices(instance->get_vulkan_data(), &gpu_count, 0));
	std::vector<VkPhysicalDevice> gpus(gpu_count);
	VKC(l->vkEnumeratePhysicalDevices(instance->get_vulkan_data(), &gpu_count, gpus.data()));
	...
	l->vkGetPhysicalDeviceQueueFamilyProperties(vulkan_data, &queue_family_count, nullptr);
	if (1 > queue_family_count) {
		LOGF(std::string("queue_family_count value is unexpected."));
	}
	queue_family_properties.resize(queue_family_count);
	l->vkGetPhysicalDeviceQueueFamilyProperties(vulkan_data, &queue_family_count, queue_family_properties.data());
	uint32_t ext_count = 0;
	l->vkEnumerateDeviceExtensionProperties(vulkan_data, nullptr, &ext_count, nullptr);
	if (ext_count > 0) {
		std::vector<VkExtensionProperties> extensions(ext_count);
		if (l->vkEnumerateDeviceExtensionProperties(vulkan_data, nullptr, &ext_count, &extensions.front()) == VK_SUCCESS) {
...
std::vector<VkSurfaceFormatKHR> gearoenix::nufrag::render::device::Physical::get_surface_formats() const {
	...
	surface->get_instance()->get_linker()->vkGetPhysicalDeviceSurfaceFormatsKHR(vulkan_data, surface->get_vulkan_data(), &count, formats.data());
	return formats;
}
