...
gearoenix::core::ecs::EntityPtr gearoenix::vulkan::model::Manager::build(std::string&& name, core::ecs::Entity* const parent, render::model::meshes_set_t&& meshes, const bool is_transformable, const bool is_skinned)
{
    auto entity = render::model::Manager::build(std::move(name), parent, std::move(meshes), is_transformable, is_skinned);
