    ...
    : logical_device(std::move(ld))
{
    ...
    const auto& rnd_cfg = phy_dev->get_surface()->get_system_application()->get_configuration().get_render();
    VkMemoryAllocateInfo info;
    GX_SET_ZERO(info)
    info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    info.allocationSize = math::Numeric::align(
        rnd_cfg.get_maximum_cpu_render_memory_size(), phy_dev->get_max_memory_alignment());
