...
gearoenix::render::command::Buffer::Buffer(const std::shared_ptr<Pool> &pool): pool(pool) {
	...
	VKC(l->vkAllocateCommandBuffers(pool->get_logical_device()->get_vulkan_data(), &cmd_buf_allocate_info, &vulkan_data));
...
void gearoenix::render::command::Buffer::flush() {
	...
	VKC(linker->vkQueueSubmit(d->get_graphic_queue(), 1, &submit_info, fence.get_vulkan_data()));
	fence.wait();
}
