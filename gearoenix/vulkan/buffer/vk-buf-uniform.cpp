...
gearoenix::nufrag::render::buffer::Uniform::Uniform(std::shared_ptr<memory::Pool> &pool, int data_size):
    ...
    VKC(l->vkCreateBuffer(logical_device->get_vulkan_data(), &buffer_info, nullptr, &vulkan_data));
    l->vkGetBufferMemoryRequirements(logical_device->get_vulkan_data(), vulkan_data, &mem_reqs);
