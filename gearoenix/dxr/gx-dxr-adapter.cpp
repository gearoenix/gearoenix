...
gearoenix::dxr::Adapter::Adapter() noexcept
{
#ifdef GX_DEBUG_MODE
    ...
    if (SUCCEEDED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&dxgi_info_queue)))) {
        GX_DXR_CHECK(CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&factory)))
...
#endif
    if (nullptr == factory) {
        GX_DXR_CHECK(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))
    }
    ...
    for (UINT adapter_id = 0; DXGI_ERROR_NOT_FOUND != factory->EnumAdapterByGpuPreference(adapter_id, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&adapter)); ++adapter_id) {
        ...
        GX_DXR_CHECK(adapter->GetDesc1(&desc))
