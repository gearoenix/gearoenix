    ...
    : Shader(eng, end)
{
    eng->add_load_function([this, end] {
        const char pvs[] = "struct VertexInputType {\n"
                           ...
                           "    outout.speculare = dot(eye_direction, reflected);\n"
        ...
        const char pfs[] = "struct PixelInputType {\n"
                           ...
                           "    if(diff_fac < 0.001) return mix(ambl_color, env_color, rfl_fac.x);\n"
                           "    float d = shdtxt.Sample(smp, input.shadow.xy).x;\n"
                           "    if(d < input.shadow.z - input.bias) return mix(ambl_color, env_color, rfl_fac.x);\n"
                           "    float spec_fac = smoothstep(spec_factors.x, spec_factors.y, input.speculare) * spec_factors.z;\n"
                           "    float4 final_color = mix(sun_color, ambl_color, diff_fac);\n"
                           "    final_color = mix(final_color, env_color, rfl_fac.x);\n"
