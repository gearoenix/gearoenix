    ...
    : Shader(eng, end)
{
    eng->add_load_function([this, end] {
		const char pvs[] =
			...
			"    float4 shadow = mul(world_position, db);\n"
			"    output.shadow = float3(shadow.xy, shadow.z / shadow.w);\n"
		...
		const char pfs[] =
			...
			"    float4 ambl_light = float4(txt_color.xyz * ambl_color, txt_color.w);\n"
			...
			"        float3 diff_color = (txt_color.xyz * diff_fac) * sun_color;\n"
			"        float3 spc_color = spec_color * spec_fac;\n"
