...
struct BloomData final {
    ...
    uint horizontal_texture = static_cast<uint>(-1);
    uint vertical_texture = static_cast<uint>(-1);
    uint source_target = static_cast<uint>(-1);
    ...
    std::array<uint, GX_RENDER_MAX_BLOOM_DOWN_SAMPLE_COUNT - 1> vertical_targets { static_cast<uint>(-1) };
    std::array<uint, GX_RENDER_MAX_BLOOM_DOWN_SAMPLE_COUNT> upsampler_targets { static_cast<uint>(-1) };
