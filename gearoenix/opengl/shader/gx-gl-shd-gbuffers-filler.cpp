...
void main() {
    ...
    frag_out_albedo_metallic = vec4(texture(albedo, out_uv).xyz * albedo_factor.xyz, metallic);
    ...
    frag_out_radiance = textureLod(radiance, normalize(reflect(out_eye, frag_out_normal_ao.xyz)), roughness * alpha_cutoff_occlusion_strength_radiance_lod_coefficient_reserved.z).xyz;
}
)SHADER";
}
