...
static constexpr auto GX_GLES3_MIN_TEX2D_ASPECT = 64;
    ...
    : render::texture::Texture2D(my_id, e)
{
    ...
    gl::uint cf;
    const gl::sizei gimg_width = GX_GLES3_MIN_TEX2D_ASPECT < img_width ? static_cast<gl::sizei>(img_width) : GX_GLES3_MIN_TEX2D_ASPECT;
    const gl::sizei gimg_height = GX_GLES3_MIN_TEX2D_ASPECT < img_height ? static_cast<gl::sizei>(img_height) : GX_GLES3_MIN_TEX2D_ASPECT;
...
gearoenix::gles3::texture::Texture2D::Texture2D(const core::Id my_id, const gl::uint txtobj, engine::Engine* const e) noexcept
    : render::texture::Texture2D(my_id, e)
    , texture_object(txtobj)
{
}

gearoenix::gles3::texture::Texture2D::~Texture2D() noexcept
{
    if (texture_object == 0)
        return;
...
void gearoenix::gles3::texture::Texture2D::bind(gl::enumerated texture_unit) const noexcept
{
