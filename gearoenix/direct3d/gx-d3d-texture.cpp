...
void gearoenix::d3d::TextureManager::wait(const UINT64 fv)
{
    ...
    const ayto wait_result = WaitForSingleObjectEx(fence_event, INFINITE, false);
    GX_ASSERT_D(WAIT_OBJECT_0 == wait_result);
    const auto close_handle = CloseHandle(fence_event);
    ...
    const core::job::EndCaller& c)
{
    ...
    GX_D3D_CHECK(device->get_device()->CreateCommittedResource(&p, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&resource)));
