...
gearoenix::d3d::Camera::Camera(Engine& e, const std::string& name) noexcept
...
gearoenix::d3d::Camera::~Camera() noexcept = default;

gearoenix::d3d::Camera::Camera(Camera&&) noexcept = default;

gearoenix::d3d::CameraBuilder::CameraBuilder(Engine& e, const std::string& name) noexcept
    : render::camera::Builder(e, name)
{
    entity_builder->get_builder().add_component(Camera(e, name));
}

gearoenix::d3d::CameraBuilder::~CameraBuilder() noexcept = default;

std::shared_ptr<gearoenix::render::camera::Builder> gearoenix::d3d::CameraManager::build(const std::string& name) noexcept
{
    return std::shared_ptr<render::camera::Builder>(new CameraBuilder(dynamic_cast<Engine&>(e), name));
}

gearoenix::d3d::CameraManager::CameraManager(Engine& e) noexcept
    : render::camera::Manager(e)
{
}

gearoenix::d3d::CameraManager::~CameraManager() noexcept = default;
