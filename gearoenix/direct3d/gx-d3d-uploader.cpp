    ...
    core::sync::EndCaller&& end) noexcept
{
    uploader.push([this, data = std::move(data), buffer = std::move(buffer), end = std::move(end)]() {
        ...
        GX_D3D_CHECK(dev->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COPY, IID_PPV_ARGS(&cal)));
        GX_D3D_CHECK(dev->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COPY, cal.Get(), nullptr, IID_PPV_ARGS(&cmd)));
    ...
    core::sync::EndCaller&& end) noexcept
{
    uploader.push([this, data = std::move(data), texture = std::move(texture), end = std::move(end), subresource_index]() {
        ...
        GX_D3D_CHECK(dev->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COPY, IID_PPV_ARGS(&cal)));
        GX_D3D_CHECK(dev->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COPY, cal.Get(), nullptr, IID_PPV_ARGS(&cmd)));
        GX_D3D_CHECK(dev->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&gcal)));
        GX_D3D_CHECK(dev->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, gcal.Get(), nullptr, IID_PPV_ARGS(&gcmd)));
        ...
        GX_D3D_CHECK(cmd->Close());
        copy_queue->exe(cmd);
        GX_D3D_CHECK(copy_queue->get_command_queue()->Signal(fence.Get(), fence_value));
        GX_D3D_CHECK(direct_queue->get_command_queue()->Wait(fence.Get(), fence_value));
        ++fence_value;
