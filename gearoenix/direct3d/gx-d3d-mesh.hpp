...
struct Mesh final : public render::mesh::Mesh {
    ...
    Mesh(std::shared_ptr<GpuBuffer>&& vb, std::shared_ptr<GpuBuffer>&& ib, math::Aabb3<core::fp_t>&& box, UINT vertex_size, UINT vertices_size, UINT indices_count);
    ~Mesh() override;
};
...
struct MeshManager final : render::mesh::Manager {
    ...
    [[nodiscard]] std::shared_ptr<render::mesh::Mesh> build(std::string&& name, std::vector<render::PbrVertex>&& vertices, std::vector<std::uint32_t>&& indices, math::Aabb3<core::fp_t>&& occlusion_box, core::job::EndCaller&& end_callback) override;
};
}
