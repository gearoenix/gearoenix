...
gearoenix::glc3::pipeline::ForwardPbrDirectionalShadowResourceSet::ForwardPbrDirectionalShadowResourceSet(const std::shared_ptr<shader::ForwardPbrDirectionalShadow>& shd)
    : glc3::pipeline::ResourceSet(shd)
{
}

void gearoenix::glc3::pipeline::ForwardPbrDirectionalShadowResourceSet::bind(gl::uint& bound_shader_program) const
{
    GX_GLC3_PIPRES_START_DRAWING_MESH
    GX_GLC3_PIPRES_START_SHADER(ForwardPbrDirectionalShadow, shd)
    ...
    GX_GLC3_PIPRES_SET_UNIFORM(effect_view_projection_biases, *(node->cascades_view_projections_bias[0].data()))
    GX_GLC3_PIPRES_SET_UNIFORM(effect_cascades_count, node->cascades_count)
