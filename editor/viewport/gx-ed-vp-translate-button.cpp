...
gearoenix::editor::viewport::TranslateButton::TranslateButton(const Button& previous)
    : previous(previous)
{
    toggled = true;
...
void gearoenix::editor::viewport::TranslateButton::update()
{
    ...
    if (clicked_in_this_frame) {
        if (toggled) {
            render::gizmo::Manager::get().enable_translation_handle();
        } else {
            render::gizmo::Manager::get().disable_translation_handle();
        }
    }
}
