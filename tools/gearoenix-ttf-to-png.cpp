...
#define ATLAS_ASPECT 1024
#define MAIN_ASPECT 1024

#define DEBUG_MODE_FONT
...
int main(int argc, char** argv)
{
    ...
    stbtt_bakedchar* bkchar = new stbtt_bakedchar[num_chars];
    ...
    unsigned char* atlas_data = new unsigned char[MAIN_ASPECT * MAIN_ASPECT];
    stbtt_BakeFontBitmap(
        ttbuffer, 0, // font location (use offset=0 for plain .ttf)
        171, // height of font in pixels
        atlas_data, MAIN_ASPECT, MAIN_ASPECT, // bitmap to be filled in
        first_char, num_chars, // characters to bake
        bkchar);
#ifdef DEBUG_MODE_FONT
    unsigned char* imgdata = new unsigned char[MAIN_ASPECT * MAIN_ASPECT * 4];
    for (int i = 0, j = 0; i < MAIN_ASPECT * MAIN_ASPECT * 4; ++i, ++j) {
        if (atlas_data[j] != 0) {
            imgdata[i] = 0XFF;
            imgdata[++i] = 0XFF;
            imgdata[++i] = 0XFF;
